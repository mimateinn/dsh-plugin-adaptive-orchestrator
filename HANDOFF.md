...
**Verification:** Plugin pnpm check 102/102 across 10 files green on b96fde7 (format/lint/typecheck/build; includes headless integration smoke). DSH claim branch: agent-team 58/58, subagent 222/222, build:lib:host green, lint/diff clean.
**Settings page (implemented 25823ac):** The original DSH Settings shell now receives an Adaptive Orchestrator section — host registers the `adaptive-orchestrator` settings namespace (schemastery, applies live) and watches it to drive runtime enablement; client registers a `settings.section` entry rendering the global toggle + sensitive allowlist via ctx.settingsScope. Hooks register regardless but stay inert until the toggle turns active. Client bundle built (lib/client). User verification in the running app requires installing the plugin bundle into DSH and refreshing the Settings shell.
