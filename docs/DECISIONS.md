...
## 2026-09-01 — Original DSH Settings page owns the global toggle
- **Why:** the user wants one page in the original Settings shell with a clickable toggle; the plugin must not invent a second settings surface.
- **Rejected:** Config-only enablement, because the user must be able to flip the switch from the UI after install; and a separate plugin-owned settings page, because the requirement is a page inside the original Settings.
- **Impact:** the host registers the `adaptive-orchestrator` settings namespace (schemastery schema, `applies: live`) and watches it to drive runtime enablement; the client registers a `settings.section` entry (id `adaptive-orchestrator`) rendering the global toggle and sensitive allowlist through `ctx.settingsScope`. Enforcement hooks register regardless of the toggle but stay inert until it turns active, so the page can enable orchestration without a restart. The profile-local file repository remains the fallback when the host has no settings service.
